...
export default {
  ...
  methods: {
    mapApiPlan(p) {
      // Use features array directly if available, otherwise parse description
      const features = p.features || (p.description || "")
        .replace(/\.$/, "")
        .split("+")
        .map((s) => s.trim())
        .filter(Boolean);
      ...
      return {
        id: p.id,
        name: p.name,
        type: p.type || p.name, // Add type field for backend
    ...
    async fetchPlans() {
      try {
        ...
        console.log('Plans fetched successfully:', apiPlans);
        this.plans = (apiPlans || []).map(this.mapApiPlan);
        this.selectedPlan = null;
      } catch (e) {
        console.error('Error fetching plans:', e);
        // Set empty plans array instead of throwing
        this.plans = [];
        this.selectedPlan = null;
      } finally {
        this.loading = false;
